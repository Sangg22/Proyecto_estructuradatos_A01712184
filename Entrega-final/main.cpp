...
std::string generateUniqueFileName() {
    ...
    fileName << "tournament_results_" << std::put_time(std::localtime(&t), "%Y%m%d_%H%M%S") << ".csv";
    return fileName.str();
}
    ...
    do {
        ...
        switch (opcion) {
            ...
            case 5:
                ...
                int csvOption;
                std::cout << "Choose an option to save results:\n";
                std::cout << "1. Create a new CSV file\n";
                std::cin >> csvOption;

                if (csvOption == 1) {
                    std::string newFileName = generateUniqueFileName();
                    tournament.updateCSV(newFileName);
                    std::cout << "Results saved to new file: " << newFileName << std::endl;
                }else {
